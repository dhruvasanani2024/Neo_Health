...
// Check if a specific slot object (with hour24/minute) is in the past for today
export function isSlotPastForToday(slot, selectedDate) {
    const now = new Date()
    const today = now.toISOString().split('T')[0]

    let checkDate = today
    if (selectedDate) {
        checkDate = typeof selectedDate === 'string' ? selectedDate : selectedDate.toISOString().split('T')[0]
    }
    ...
    if (slot.hour24 < currentHour) return true
    if (slot.hour24 === currentHour && slot.minute <= currentMinute) return true
    return false
}
